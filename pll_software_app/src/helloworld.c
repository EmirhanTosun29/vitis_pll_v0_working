...
int main()
{
    ...
    xil_printf("\r\n=== SW PLL benchmark (HDL-compatible I/O) + SETTLE + TRACE ===\r\n");
    ...
    dump_word("SINE", 256, bram[256]);
    dump_word("SINE", 512, bram[512]);
    dump_word("SINE", 768, bram[768]);
    ...
    // 4) SETTLE (measurement dışı)
    const int SETTLE_SAMPLES = 5000; // 0.5 s @ 40kHz
    uint32_t settle_ms = (uint32_t)(((uint64_t)SETTLE_SAMPLES * 1000ull) / (uint64_t)FS_HZ);
    xil_printf("Settle running... samples=%d (~%lu ms)\r\n", SETTLE_SAMPLES, (unsigned long)settle_ms);
...
#if TRACE_SETTLE
        int do_print = 0;
        if (i < TRACE_EARLY_SAMPLES) {
            do_print = ((i % TRACE_EARLY_STRIDE) == 0);
        } else {
            do_print = ((i % TRACE_STRIDE) == 0);
        }
        if (i == (SETTLE_SAMPLES - 1)) do_print = 1;

        if (do_print) {
            xil_printf("[settle %6d] ", i);
            print_qn("Out_f(Hz)", st.out_f_q25, 25);
            xil_printf("  ");
            print_qn("theta(turn)", (int32_t)st.theta_q30, 30);
            xil_printf("  x_q22=0x%08lx\r\n", (unsigned long)x_q22);
        }
#endif
    }

    // 5) BENCHMARK (print yok; ölçüm temiz)
    const int N = 1024;
    uint64_t t0, t1;
    ...
    xil_printf("Made by Emirhan Tosun and Yusuf Kılıç.\r\n");
